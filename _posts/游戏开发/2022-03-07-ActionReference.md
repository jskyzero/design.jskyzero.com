...
广义上，任何以动作为主要表现内容的游戏，都可以算作动作游戏。狭义上，我会认为动作游戏：1.攻击判定遵循严格框体检测。2.受击动画清晰明确。3.有浮空、投技、眩晕之类的典型体验。


### 典型体验

1. 远离攻击：使用若干手段远离或者抵消减弱敌方攻击。
2. 打点控制：（精确地）调整人物位置、攻击朝向等，使得下次攻击刚好命中目标部位
3. 按键（离散按下）
4. 搓招/Combo类：指定时间窗口连续完成一系列指令输入
5. 当身技/返技类：配合敌方动作判定，自身动作可以产生额外效果
6. 决策（脑内思考）
7. 抉：结合当前情景，从候选行为中选择最优行为
8. 量表计算类：（显性或者隐性地）统计攻击造成的计数量表增加，并合理利用计数满额后的硬直等。
