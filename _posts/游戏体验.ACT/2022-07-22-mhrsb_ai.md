...
这部分开始就比较虚了，我们会先根据经验值设定一个“起点”，再选取几只经典的怪物的逐只看看，逐步去修正我们的“起点”。

![](/assets/img/gameplay/mhrsb.AI/3.jpg)

参考上图，考虑到怪物猎人是一个规则设计非常繁多的游戏，以下内容我们还是希望聚焦到**战斗中的“出招设计”**上。

我们先把怪物的AI行为分为战斗和其他两部分。（参考下图上半部分）

根据体验，怪物一般会有正常状态、愤怒状态（、疲劳状态）三种，同时怪物可能还有自身的强化状态的逻辑（参考下图下半部分）：
...
小小总结一下，大概可以参考下图所示的行为树（虽然就效果而言，感觉多数情况下处于某种情形都会释放某种攻击像是状态机的优势……）：
