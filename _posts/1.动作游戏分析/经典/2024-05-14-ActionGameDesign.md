...
> 这里可以带入一些没那么好的游戏作为反向例子理解，比如：当一些游戏的动画看不清，就会影响玩家对信息的获取，然后干扰玩家的决策，导致结果不符合预期，给玩家带来不好的体验。

![](/assets/img/act/2022/09.png)
...
![](/assets/img/act/2022/10.png)


#### 让奖惩设计长期有效

奖惩导向会让玩家**自然而然的去追求奖励收益最大化，如何让这个行为长期有效**？这里有一些具体的思路参考：
...
+ 控制感：内涵较为丰富，1.符合直觉的控制 2.丰富的控制选项 3.丰富的考核场景（带来不同的正面应对意义）
+ 打击感：聚焦于动作本身的美学呈现。
...
这里补充一些典型的认知到的行为的目标，和目标可能侧重的层次，如下：


1. 【感知】精准交互类：配合敌方动作判定，释放动作，一般可以产生额外效果，比如当身/返技/弹反/防反/GP等等
2. 【认知】抉：结合当前情景，多为敌我位置、状态，从可选行为中选择最优行为
3. 【认知】量表计算类：显性或者隐性地统计攻击等造成的计数量表增加或者减少，并合理利用计数满额后的效果比如产生硬直等
4. 【行动】移动-回避类：使用某些手段远离某个区域——多为攻击区域
5. 【行动】移动-进攻类：精确地调整人物位置、朝向等，使得下次行为刚好命中目标或目标特定部位
6. 【行动】连招/Combo：连续完成一系列指令输入，通常控制在僵直时窗口内，输入方式可能会需要摇杆组合按键——搓招
