...
|主要层次|体验类别|说明|
|---|---|--|
|行动|移动-回避|使用某些手段远离某个区域——多为攻击区域|
|行动|移动-进攻|精确地调整人物位置、朝向等，使得下次行为刚好命中目标（或目标特定部位）|
|行动|连招类|连续完成一系列指令输入，通常控制在僵直时窗口内，输入方式可能会需要摇杆组合按键（搓招）|
|感知|敌我交互类|配合敌方动作判定，释放动作，一般可以产生额外效果，比如当身/返技/弹反/防反/GP等等|
