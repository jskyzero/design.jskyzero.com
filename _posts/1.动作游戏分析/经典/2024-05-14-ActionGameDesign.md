...
1. 感知
    1. 精准交互类：配合敌方动作判定，释放动作，一般可以产生额外效果，比如当身/返技/弹反/防反/GP等等
2. 认知
    1. 抉：结合当前情景（多为敌我位置、状态），从可选行为中选择最优行为
    2. 量表计算类：（显性或者隐性地）统计攻击等造成的计数量表增加或者减少，并合理利用计数满额后的效果比如产生硬直等
3. 行动
    1. 移动-回避类：使用某些手段远离某个区域——多为攻击区域
    2. 移动-进攻类：精确地调整人物位置、朝向等，使得下次行为刚好命中目标（或目标特定部位）
    3. 连招/Combo：连续完成一系列指令输入，通常控制在僵直时窗口内，输入方式可能会需要摇杆组合按键（搓招）
